...
namespace
{
    ...
    bool calculate_passed(std::string yes_votes, std::string no_votes, const zera_txn::InstrumentContract &contract, bool fast_quorum = false)
    {
        ...
        {
            if (!db_contract_supply::get_single(contract.contract_id(), max_data) || !max_supply.ParseFromString(max_data))
            {
                return false;
            }
            if (max_supply.circulation() == "0")
        ...
        uint256_t yes_percent = (yes * 10000) / total_votes;
