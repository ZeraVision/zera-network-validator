...
namespace
{
    ...
    void something(std::string &wallet_adr, std::string &voting_id, zera_validator::Proposal &proposal, zera_txn::GovernanceVote &client_vote)
    {
        ...
        {
            logging::print("Proposal wallet does not exist:", base58_encode(wallet_adr));
        }

        bool change_vote = false;
        ...
        if (vote_wallet.proposal_votes().count(base58_encode(client_vote.proposal_id())) > 0)
        {
            if (proposal.options_set())
            {
                ...
                if (client_vote.support_option() == old_support_option)
                {
                    return;
                }
                change_vote = true;
            }
            else if (client_vote.support() == vote_wallet.proposal_votes().at(base58_encode(client_vote.proposal_id())).support())
            {
                return;
            }
            else
        ...
        // get amount of voting instrument client has
        if (db_wallets::get_single(wallet_adr + voting_id, client_amount))
        {
            ...
            if (change_vote)
            {
                remove_votes(client_votes, proposal, voting_id, client_vote.support(), old_support_option);
            }
            ...
            logging::print("governance txn size:", std::to_string(proposal.governance_txn_size()));
        ...
        if (txn_passed.at(client_vote.base().hash()))
        {
            ...
            if (db_proposals::get_single(client_vote.proposal_id(), votes_data) && proposal.ParseFromString(votes_data))
            {
                ...
                for (auto wallet_adr : delegate_wallets.wallets())
                {
                    ...
                    {
                        something(wallet_adr, voting_id, proposal, client_vote);
                    }
                }

                for (auto voting_id : proposal_contract.governance().voting_instrument())
                {
                    something(client_vote_adr, voting_id, proposal, client_vote);
                }
            }
